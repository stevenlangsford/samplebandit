...
## state_of_play_plot <- function(samples,calcobshistory,infohistory){
##     (check_values(samples)+check_features(samples))/(
##         ggplot(infohistory,aes(x=timestep,y=infovalue,color=whichoption,shape=whichfeature))+geom_point()+geom_line()
##         )
## }
...
for(i in 1:total_obs){
    ...
    infohistory.df <- rbind(infohistory.df,
                            data.frame(n_obs=nrow(calcobs.df),
