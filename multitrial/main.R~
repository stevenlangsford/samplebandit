...
while(obscounter <= obs_to_decision){ #for loop annoying with crash restoration? Is while actually better, or did you just miswrite/misread here?
    ...
    if(length(list.files(path,pattern="ord"))>0){
    ...
    }#end if there are files to restore in obsdump

    samples <- getsamples(calcobs.df,ordobs.df) #current state of belief
    ...
    obscounter <- obscounter+1 #you're a for loop now, harry.
