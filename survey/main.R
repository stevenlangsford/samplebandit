...
verbose <- TRUE #breadcrumb trail of print statements... why can't R give line numbers on errors?

##Things you want to survey over:
for(calcnoise in c(.5)){
    for(ordnoise in c(.5,1)){
        for(obs_per_imaginationcycle in c(10,20)){ #is 20 way too much?
            for(stim_displace_x in c(0,-1)){#the prior is at 0,0
                for(stim_displace_y in c(0,-1)){

                    decision_trajectory.df <- data.frame()
                    source("stim_setup/context_stimsetup.R") #everything here assumes stim.df exists, is visible in global env. stimsetup also loads libraries,sets seed.
                    ##apply any displacement:
                    for(i in 1:nrow(stim.df)){
                        if(stim.df$feature[i]==1){
                            stim.df$value[i] <- stim.df$value[i]+stim_displace_x
                        }else{
                            stim.df$value[i] <- stim.df$value[i]+stim_displace_y
                        }
                    }

                    runname <- paste0("survey_cn",calcnoise,"on",ordnoise,"opic",obs_per_imaginationcycle,"dx",stim_displace_x,"dy",stim_displace_y) #used in path/filename when saving
...
for(obsrep in 1:obs_per_trial){
    if(verbose)print(paste("Start of observation round",obsrep, "for",runname))#derpy progress bar
...
finishtime <- Sys.time()
print(paste(runname,"got done in"))
