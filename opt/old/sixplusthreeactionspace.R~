...
juice_a_stim <- function(simtruth, myview){
    ...
    obs11 <- get_state(observe(simtruth, myview, 1, 1))
    obs12 <- get_state(observe(simtruth, myview, 1, 2))
    obs21 <- get_state(observe(simtruth, myview, 2, 1))
    obs22 <- get_state(observe(simtruth, myview, 2, 2))
    obs31 <- get_state(observe(simtruth, myview, 3, 1))
    obs32 <- get_state(observe(simtruth, myview, 3, 2))
...
for (myview in unique(results.df$viewstate)){
    ...
    for (i in 1:6){
        ...
        guessnext <- guessnext[guessnext != 0] / sum(guessnext)
         reachable.df[1, names(nextstates)[i]] <-  paste(
